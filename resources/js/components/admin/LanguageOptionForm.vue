<script setup lang="ts">
import { Form } from '@inertiajs/vue3';
import InputError from '@/components/InputError.vue';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Spinner } from '@/components/ui/spinner';

type LanguageOptionFormData = {
    code: string;
    label: string;
    native_label: string | null;
    sort_order: number;
    is_active?: boolean;
};

const props = withDefaults(
    defineProps<{
        action: string;
        idPrefix: string;
        method: 'post' | 'patch';
        submitLabel: string;
        language?: LanguageOptionFormData;
        showActiveField?: boolean;
    }>(),
    {
        language: () => ({
            code: '',
            label: '',
            native_label: null,
            sort_order: 0,
            is_active: true,
        }),
        showActiveField: false,
    },
);
</script>

<template>
    <Form
        :action="props.action"
        :method="props.method"
        class="space-y-5"
        v-slot="{ errors, processing }"
    >
        <div class="grid gap-4 sm:grid-cols-2">
            <div class="grid gap-2">
                <Label :for="`${props.idPrefix}-code`">Code</Label>
                <Input
                    :id="`${props.idPrefix}-code`"
                    name="code"
                    type="text"
                    maxlength="20"
                    required
                    autocomplete="off"
                    placeholder="de"
                    :default-value="props.language.code"
                />
                <InputError :message="errors.code" />
            </div>

            <div class="grid gap-2">
                <Label :for="`${props.idPrefix}-sort-order`">
                    Sortierung
                </Label>
                <Input
                    :id="`${props.idPrefix}-sort-order`"
                    name="sort_order"
                    type="number"
                    min="0"
                    step="1"
                    required
                    :default-value="props.language.sort_order"
                />
                <InputError :message="errors.sort_order" />
            </div>
        </div>

        <div class="grid gap-4 sm:grid-cols-2">
            <div class="grid gap-2">
                <Label :for="`${props.idPrefix}-label`">Sprache</Label>
                <Input
                    :id="`${props.idPrefix}-label`"
                    name="label"
                    type="text"
                    maxlength="80"
                    required
                    autocomplete="off"
                    placeholder="Deutsch"
                    :default-value="props.language.label"
                />
                <InputError :message="errors.label" />
            </div>

            <div class="grid gap-2">
                <Label :for="`${props.idPrefix}-native-label`">
                    Native Bezeichnung
                </Label>
                <Input
                    :id="`${props.idPrefix}-native-label`"
                    name="native_label"
                    type="text"
                    maxlength="80"
                    autocomplete="off"
                    placeholder="Deutsch"
                    :default-value="props.language.native_label ?? ''"
                />
                <InputError :message="errors.native_label" />
            </div>
        </div>

        <div
            v-if="props.showActiveField"
            class="flex items-center gap-3 rounded-md border border-border bg-background/60 px-3 py-3 dark:bg-input/20"
        >
            <input type="hidden" name="is_active" value="0" />
            <input
                :id="`${props.idPrefix}-is-active`"
                name="is_active"
                type="checkbox"
                value="1"
                :checked="props.language.is_active ?? true"
                class="size-4 rounded border-input accent-primary"
            />
            <Label :for="`${props.idPrefix}-is-active`">
                Sprache direkt aktivieren
            </Label>
        </div>

        <InputError v-if="props.showActiveField" :message="errors.is_active" />

        <Button type="submit" :disabled="processing">
            <Spinner v-if="processing" />
            {{ props.submitLabel }}
        </Button>
    </Form>
</template>
