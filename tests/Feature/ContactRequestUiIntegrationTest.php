...
test('contact action UI uses existing inertia endpoints for every status', function () {
    ...
    expect($actions)
        ->toContain('followAction(username, isFollowing)')
        ->toContain("status === 'none'")
        ->toContain('sendContactRequestAction')
        ->toContain("status === 'outgoing_request'")
        ->toContain('relationshipActionUnavailableText.contactRequestSent')
        ...
        ->toContain(':method="contactMessageAction(userId).method"')
